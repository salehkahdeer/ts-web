...
namespace Seed.TriumphSkill.ViewModels
{
    public class DataTablesPageRequest
    {
        ...
        public IEnumerable<SortCondition> GetSortConditions()
        {
            if (this.SortCol.Count < 1)
                return null;

            List<string> columnNames = this.ColumnNames.Split(',').ToList();

            var sortConditions = columnNames.Where(column => this.SortCol.Contains(columnNames.IndexOf(column)))
                                           .Select(column => new SortCondition
                                           {

                                               ColumnName = column,

                                               SortDirection = this.SortDir[this.SortCol.IndexOf(this.SortCol.First(i => i == columnNames.IndexOf(column)))].ToUpper() == "ASC" ? SortDirection.Ascending : SortDirection.Descending

                                           }).ToList<SortCondition>();
